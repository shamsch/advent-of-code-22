...
func comparePairsForOverlap(pair1 Pairs, pair2 Pairs) bool {
	if (pair1.Start <= pair2.Start && pair1.End >= pair2.Start) || (pair1.Start <= pair2.End && pair1.End >= pair2.End) { 
		return true
	} else if (pair2.Start <= pair1.Start && pair2.End >= pair1.Start) || (pair2.Start <= pair1.End && pair2.End >= pair1.End) {
		return true
	} else {
		return false
	}
...
func main() {
	...
	for _, line := range lines {
		...
		if comparePairsForOverlap(firstPairStruct, secondPairStruct) {
			sum++
		}
	}
